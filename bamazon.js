...
function updateUser(attr,value,userid){
 ...
 var newValue=moment().format("MM/DD/YYYY");
 var attribute=attr;
 var sql ='UPDATE BAMAZON_DB.CUSTOMERS SET LAST_LOGIN_DT=?  WHERE ??=?'

    connection.query(sql,[newValue,'CUST_ID',user], function (error, results, fields) {
        if (error){ 
            throw error;
...
function addUser(valuesArray){
    ...
    var sql="INSERT INTO bamazon_db.customers (EMP_ID, USER_ID, USER_NAME, USER_ROLE, PW,LAST_LOGIN_DT) VALUES ?";
       ...
       inquirer.prompt({
         ...
         name: "action"
           ...
           if(response.action=='Log Out'){
               ...
               console.log("Sorry! That isn't functional yet.");
               getMgrAction();
           }
       });
   }
...
function buySomethingElse(){
        ...
        showProducts();
           console.log("\n");
           inquirer.prompt([{name:"itemcd",type:"input",message:"What is the item code you would like to purchase?"},
           {name:"qty",type:"input",message:"How many would you like?"}],
           (err)=>{
               console.log("It appears you are having difficulty. Please contact the help desk.");
               process.exit(99);
